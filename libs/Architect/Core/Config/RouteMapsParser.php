...
class RouteMapsParser {
	...
	private function parseRouteMaps() {
		...
		if($nodes->length > 0) {
			...
			foreach($nodes as $current_index => $node) {
				...
				$default_route_ssl_option = $this->parser->query('//arch:settings/arch:ssl', null, true)->getAttribute('mode', true, '/^(allow|enforce|restrict)$/');
				...
				$route_ssl_option = $node->getAttribute('ssl', true, '/^(allow|enforce|restrict)$/');
				$route_ssl_option = (strlen($route_ssl_option) > 0) ? $route_ssl_option : $default_route_ssl_option;
				
				// Set route SSL option
				$this->parsed_route_maps[$this->current_route_map_pattern]['ssl'] = $route_ssl_option;
		...
		// Log memory usage of parsed route maps
		\Jarvis\Memory::log($parsed_route_maps, 'Size of parsed route maps.', 'Route Maps', __FILE__, __LINE__);

		// Log total excecution time
		\Jarvis\Benchmark::assert('RouteMaps');
	...
	protected function parseRouteAction() {
		...
		// Fetch action node
		$node = $this->parser->query('arch:action', $this->current_node, true);
		...
		$this->parsed_route_maps[$this->current_route_map_pattern]['action'] = array(
			'name' => $action,
			'callback' => $callback,
	 ...
	 *
	 *	@throws Exceptions\RouteMapsParserException
	 *
	 *	@return void
	 */
